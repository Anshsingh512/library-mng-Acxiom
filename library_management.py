...
class Member:
    ...
    def calculate_fine(self, book_id):
        ...
        for book, issue_date in self.issued_books:
            if book.book_id == book_id:
                days_overdue = (datetime.now() - issue_date).days - 14  # 14 days allowed
                if days_overdue > 0:
...
class Library:
    ...
    def save_members_to_csv(self):
        with open('members.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['member_id', 'name', 'membership_type', 'fines'])
            for member_id, member in self.members.items():
                writer.writerow([member.member_id, member.name, member.membership_type, member.fines])
    ...
    if library.authenticate(username, password) == "admin":
        ...
        while True:
            ...
            if choice == "1":
                ...
                copies = input("Enter new number of copies (or press enter to skip): ")
                ...
                issued_books = library.list_issued_books()
                if issued_books:
                    print("Issued Books:")
                    for book, member_name, issue_date in issued_books:
                ...
                if overdue_books:
                    print("Overdue Books:")
                    for book, member_name, fine in overdue_books:
                        print(f"{book} is overdue by {member_name}. Fine: ${fine}")
                else:
